...
# IDs
treated_id <- 22
control_ids <- c(1:10,12:18,19,20,21, 24:31)
all_ids <- c(treated_id, control_ids)

# Common settings
pre_period  <- 2000:(treated_year - 1)
post_period <- treated_year:2019
time_plot   <- 2000:2019

# A small, reusable dataprep wrapper
make_dataprep <- function(treat_id, donors){
  dataprep(
    foo = SCM,
    predictors = SCpredictors,
    predictors.op = "mean",
    dependent = "income_ratio",
    unit.variable = "code",
    time.variable = "Year",
    special.predictors = list(
      list("income_ratio", 2001, "mean")
    ),
...
# Run Synth safely and compute gaps + RMSPEs
run_one_scm <- function(treat_id, donors){
  dp <- make_dataprep(treat_id, donors)
  fit <- tryCatch(synth(data.prep.obj = dp), error = function(e) NULL)
  if (is.null(fit)) return(NULL)

  # Actual and synthetic paths
  # Y1plot: T x 1; Y0plot: T x J; weights: J x 1
  Y1  <- as.numeric(dp$Y1plot)
  Y0w <- as.numeric(dp$Y0plot %*% fit$solution.w)

  gap <- Y1 - Y0w
  names(gap) <- time_plot

  # RMSPEs
  pre_idx  <- which(time_plot %in% pre_period)
  post_idx <- which(time_plot %in% post_period)

  pre_rmspe  <- sqrt(mean(gap[pre_idx]^2,  na.rm = TRUE))
  post_rmspe <- sqrt(mean(gap[post_idx]^2, na.rm = TRUE))
  ratio <- ifelse(pre_rmspe > 0, post_rmspe / pre_rmspe, NA_real_)

  # Weights table (optional; helpful for diagnostics)
  w_tbl <- tibble(
    treated = treat_id,
    donor_code = as.integer(colnames(dp$X0)),    # donor codes (as character) from X0 colnames
    weight = as.numeric(fit$solution.w)
  ) |>
    filter(weight > 0)

  list(
    treat_id = treat_id,
    unit_name = as.character(unique(SCM$Region[SCM$code == treat_id])),
    gap = tibble(Year = time_plot, gap = gap, treat_id = treat_id),
    pre_rmspe = pre_rmspe,
...
# For each unit k, treat k as "treated" and use all others as donors
results_list <- map(all_ids, function(k) {
  donors_k <- setdiff(all_ids, k)
  run_one_scm(k, donors_k)
})
...
# Tidy outputs
gap_df <- map_dfr(results_list, "gap")
rmspe_df <- map_dfr(results_list, ~tibble(
  treat_id = .x$treat_id,
  unit_name = .x$unit_name,
  pre_rmspe = .x$pre_rmspe,
  post_rmspe = .x$post_rmspe,
  rmspe_ratio = .x$ratio
))

weights_df <- map_dfr(results_list, "weights")

# Identify the "true treated"
treated_ratio <- rmspe_df |> filter(treat_id == treated_id) |> pull(rmspe_ratio)
treated_pre   <- rmspe_df |> filter(treat_id == treated_id) |> pull(pre_rmspe)
treated_post  <- rmspe_df |> filter(treat_id == treated_id) |> pull(post_rmspe)

# Exact permutation p-value (two-sided on ratio's right tail by convention)
# p = share of placebo units with ratio >= treated ratio
perm_p <- mean(rmspe_df$rmspe_ratio >= treated_ratio, na.rm = TRUE)

# Save core tables
write_csv(rmspe_df, "SCM_placebo_permutation_RMSPE.csv")
write_csv(weights_df, "SCM_placebo_weights.csv")

# Messages for the console/log
cat(sprintf("Treated unit = %s\n", unique(rmspe_df$unit_name[rmspe_df$treat_id == treated_id])))
cat(sprintf("Pre RMSPE (treated):  %.4f\n", treated_pre))
cat(sprintf("Post RMSPE (treated): %.4f\n", treated_post))
cat(sprintf("RMSPE Ratio (treated): %.4f\n", treated_ratio))
cat(sprintf("Permutation p-value (>= treated ratio): %.3f\n", perm_p))



gap_df <- gap_df |>
  mutate(group = ifelse(treat_id == treated_id, "Treated", "Placebo"))

vline_x <- treated_year

p_spaghetti <- ggplot(gap_df, aes(x = Year, y = gap, group = treat_id, alpha = group)) +
  geom_line(color = "black") +
  scale_alpha_manual(values = c(Placebo = 0.25, Treated = 1.0), guide = "none") +
  geom_vline(xintercept = vline_x, linetype = "dashed") +
  labs(x = "Year", y = "Gap: Actual − Synthetic (income_ratio)",
       title = "Placebo-in-Space Gaps",
       subtitle = paste0("Dashed line = treatment year (", treated_year, ")")) +
  theme_minimal(base_size = 12)

ggsave("SCM_spaghetti_gaps.png", p_spaghetti, width = 8, height = 5, dpi = 300)
p_spaghetti




### Distribution of RMSPE ratios (permutation test visualization)
p_hist <- rmspe_df |>
  ggplot(aes(x = rmspe_ratio)) +
  geom_histogram(bins = 20) +
  geom_vline(xintercept = treated_ratio, linetype = "dashed") +
  annotate("text",
           x = treated_ratio, y = Inf, vjust = 1.5, hjust = -0.05,
           label = paste0("Treated ratio = ", round(treated_ratio, 3))) +
  labs(x = "Post/Pre RMSPE ratio",
       y = "Count",
       title = "Permutation Test: RMSPE Ratio Distribution",
       subtitle = paste0("Exact p-value (>= treated) = ", sprintf("%.3f", perm_p))) +
  theme_minimal(base_size = 12)

ggsave("SCM_permutation_RMSPEratio_hist.png", p_hist, width = 7, height = 5, dpi = 300)
p_hist


### Optional: Exclude poor pre-fit placebos (Abadie et al. practice)

### It’s common to drop placebo units with very poor pre-treatment fit (e.g., pre-RMSPE larger than 5× the treated unit’s pre-RMSPE). This avoids visually dominating the spaghetti plot and focuses inference on credible placebos.
# Filter rule: keep placebo if pre RMSPE <= c * treated_pre

c_cut <- 5
good_ids <- rmspe_df |>
  filter(treat_id == treated_id | pre_rmspe <= c_cut * treated_pre) |>
  pull(treat_id)

gap_df_good <- gap_df |> filter(treat_id %in% good_ids)

p_spaghetti_good <- ggplot(gap_df_good, aes(x = Year, y = gap, group = treat_id,
                                            alpha = (treat_id == treated_id))) +
  geom_line(color = "black") +
  scale_alpha_manual(values = c(`TRUE` = 1.0, `FALSE` = 0.25), guide = "none") +
  geom_vline(xintercept = treated_year, linetype = "dashed") +
  labs(x = "Year", y = "Gap: Actual − Synthetic (income_ratio)",
       title = "Placebo-in-Space Gaps (Filtered by Pre-RMSPE)",
       subtitle = paste0("Kept placebos with pre-RMSPE ≤ ", c_cut, " × treated pre-RMSPE")) +
  theme_minimal(base_size = 12)

ggsave("SCM_spaghetti_gaps_filtered.png", p_spaghetti_good, width = 8, height = 5, dpi = 300)
p_spaghetti_good
